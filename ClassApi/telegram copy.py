...
class TelegramBot:
    ...
    async def handle_echo(self, message: types.Message):
        ...
        if user_id in self.chatgpt_active_users:
            ...
            if received_text == "/hola":
                ...
                self.spacy_processor = SpacyProcessor(user_spacy_model)
                doc = self.spacy_processor.analyze_message(received_text)

                # Generar la respuesta basada en el análisis de spaCy
                if doc["entities"]:
                    response = f"¡Genial! Noté que mencionaste {', '.join(doc['entities'])}. ¿Podrías contarme más sobre eso? Me encanta aprender cosas nuevas de mis amigos."
                elif doc["nouns"] and doc["adjectives"] and doc["verbs"]:
                    response = f"¡Vaya! Me gustaría saber más sobre ese {', '.join(doc['adjectives'])} {doc['nouns'][0]} y por qué {doc['verbs'][0]} es tan importante para ti. ¡Cuéntame más!"
                elif doc["nouns"] and doc["adjectives"]:
                    response = f"¡Interesante! ¿Podrías decirme más sobre ese {', '.join(doc['adjectives'])} {doc['nouns'][0]}? Me encanta saber más sobre tus intereses."
                elif doc["adverbs"]:
                    response = f"¡Eso suena intrigante! ¿Podrías explicarme más sobre cómo {', '.join(doc['adverbs'])} está relacionado con lo que estamos hablando?"
                elif doc["pronouns"]:
                    response = f"¿De quién estás hablando con {', '.join(doc['pronouns'])}? Me gustaría saber más para entenderte mejor."
                elif doc["prepositions"]:
                    response = f"{', '.join(doc['prepositions'])}... ¡Qué curioso! ¿Podrías darme más detalles para entender mejor?"
                elif doc["conjunctions"]:
                    response = f"¡Me encanta cómo {', '.join(doc['conjunctions'])} conecta tus ideas! ¿Podrías compartir más?"
                else:
                    await self.typing_indicator(user_id)
                    additional_question = self.messagedefault['messages']['additional_question']
                    response = additional_question

                # Añade contexto a la respuesta en base al último mensaje relevante
                last_relevant_message = await self.get_last_relevant_message(user_id, received_text)
                if last_relevant_message:
                    response = f"Me viene a la mente que hablaste de '{last_relevant_message}'. {response}"
                else:
                    response = f"¡Genial! Noté que mencionaste {received_text}. ¿Podrías contarme más sobre eso? Me encanta aprender cosas nuevas de mis amigos."
